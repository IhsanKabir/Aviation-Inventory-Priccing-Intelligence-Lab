"""
Discover US-Bangla (BS) routes from the TTInteractive bootstrap config.

Examples:
  python tools/discover_bs_routes.py --domestic-only
  python tools/discover_bs_routes.py --origin DAC --origin CGP --output output/bs_routes.json
"""
...
def main() -> None:
    ...
    entries = bs.discover_route_entries(
    ...
    if args.output:
        ...
        print(f"Wrote {len(entries)} BS routes to {out_path}")
        return
