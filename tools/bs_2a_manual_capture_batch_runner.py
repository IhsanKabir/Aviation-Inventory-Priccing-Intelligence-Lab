...
RUNNER_TOOL = REPO_ROOT / "tools" / "bs_2a_manual_capture_runner.py"
...
def _iter_csv_rows(path: Path) -> Iterable[dict[str, Any]]:
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        # Skip blank and comment lines before feeding DictReader
...
def _normalize_job(raw: dict[str, Any], *, default_carrier: str | None, defaults: argparse.Namespace) -> dict[str, Any]:
    ...
    carrier = str(pick("carrier", "airline") or default_carrier or "").strip().upper()
    ...
    if carrier not in {"BS", "2A"}:
        raise SystemExit(f"Invalid/missing carrier in queue row: {raw!r}")
...
def _find_run_dir_for_job(session_root: Path, job: dict[str, Any], started_ts: float) -> Path | None:
    ...
    if not runs_dir.exists():
        return None
    prefix = f"{job['carrier'].lower()}_{job['origin']}_{job['destination']}_{job['date']}_".lower()
    candidates = []
    for d in runs_dir.iterdir():
        ...
        if not d.name.lower().startswith(prefix):
            continue
        # allow slight clock/mtime jitter
...
def _load_run_artifacts(run_dir: Path | None, carrier: str) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    if not run_dir:
        return None, None
    slug = carrier.lower()
    probe = run_dir / f"{slug}_probe_response.json"
    ingest = run_dir / f"{slug}_manual_ingest_result.json"
    probe_obj = None
    ingest_obj = None
    try:
        if probe.exists():
...
def _runner_cmd_for_job(args: argparse.Namespace, job: dict[str, Any]) -> list[str]:
    runner_python = args.runner_python or "python"
    cmd: list[str] = [str(runner_python), str(RUNNER_TOOL)]
    _add_arg(cmd, "--carrier", job["carrier"])
    ...
    _add_arg(cmd, "--session-bundle-in", args.session_bundle_in)
    _add_arg(cmd, "--session-bundle-out", args.session_bundle_out)
    ...
    if args.ingest_python:
        _add_arg(cmd, "--ingest-python", args.ingest_python)
    return cmd


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Sequential queue runner for BS/2A manual-assisted capture (+ optional auto-ingest).",
        epilog=(
            "Queue file formats:\n"
            "  CSV headers: carrier,origin,destination,date[,cabin,adt,chd,inf,note]\n"
            "  JSON: [{...}, {...}] or {\"jobs\": [{...}]}\n"
            "\n"
            "Typical use:\n"
            "  python tools/bs_2a_manual_capture_batch_runner.py --queue-file jobs.csv --cdp-url http://127.0.0.1:9222 --ingest\n"
        ),
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument("--queue-file", required=True, help="CSV/JSON file listing jobs")
    parser.add_argument("--carrier", choices=["BS", "2A"], help="Default carrier if queue rows omit carrier")
    ...
    parser.add_argument("--session-bundle-in")
    parser.add_argument("--session-bundle-out")
    ...
    parser.add_argument("--ingest-allow-mismatch", action="store_true")
    parser.add_argument("--ingest-python", help="Python executable for the ingest step inside the capture runner")
    parser.add_argument("--runner-python", help="Python executable to run bs_2a_manual_capture_runner.py (default: current interpreter)")
    ...
    jobs = [_normalize_job(r, default_carrier=args.carrier, defaults=args) for r in raw_rows]
    ...
    for i, job in enumerate(jobs, start=1):
        print(f"[batch] {i:03d}/{len(jobs)} {job['carrier']} {job['origin']}->{job['destination']} {job['date']} cabin={job['cabin']}")
    ...
    if args.dry_run_queue:
        ...
        print(f"[batch] Dry-run only. Wrote queue summary: {result_out}")
        return 0
    ...
    for idx, job in enumerate(jobs, start=1):
        print("")
        print("=" * 80)
        print(f"[batch] Starting job {idx}/{len(jobs)}: {job['carrier']} {job['origin']}->{job['destination']} {job['date']}")
        if job.get("note"):
            print(f"[batch] Note: {job['note']}")
        started_ts = time.time()
        started_utc = _now_utc_iso()
        cmd = _runner_cmd_for_job(args, job)
        if args.print_command:
            print("[batch] Runner command:")
            print("  " + subprocess.list2cmdline(cmd))
        proc = subprocess.run(cmd, cwd=str(REPO_ROOT))
        ended_utc = _now_utc_iso()

        run_dir = _find_run_dir_for_job(session_root, job, started_ts)
        probe_obj, ingest_obj = _load_run_artifacts(run_dir, job["carrier"])
        ...
        if probe_obj:
            result_item["probe_summary"] = {
                ...
                "ok": probe_obj.get("ok"),
                "datadome_blocked": probe_obj.get("datadome_blocked"),
                "reached_search_result_url": probe_obj.get("reached_search_result_url"),
                "reached_flexibleflightliststatic_url": probe_obj.get("reached_flexibleflightliststatic_url"),
        ...
        if ingest_obj:
            result_item["ingest_summary"] = {
                ...
                "rows_valid_for_core": ingest_obj.get("rows_valid_for_core"),
        ...
        print(f"[batch] {status_label}: job {idx}/{len(jobs)} rc={rc}{extra}")

        # Persist after each job so progress survives interruptions.
        ...
        if not is_ok and args.stop_on_error:
            print("[batch] Stopping due to --stop-on-error")
            break
        if idx < len(jobs) and args.sleep_sec > 0:
            print(f"[batch] Sleeping {args.sleep_sec:.1f}s before next job...")
