"""
US-Bangla (BS) TTInteractive bootstrap + fetch_flights() scaffold.
...
- Route discovery helper for generating/refreshing BS routes
"""
...
BASE_URL = "https://fo-usba.ttinteractive.com"
INDEX_URL = f"{BASE_URL}/Zenith/FrontOffice/usbangla/en-GB/BookingEngine/IndexRemoveIframe"
USER_AGENT = (
...
ENV_COOKIES_PATH = "BS_COOKIES_PATH"
ENV_PROXY_URL = "BS_PROXY_URL"
ENV_SOURCE_MODE = "BS_SOURCE_MODE"
ENV_FALLBACK_ON_EMPTY = "BS_BDFARE_FALLBACK_ON_EMPTY"
...
def _sharetrip_fetch_with_bdfare_fallback(
    ...
    # Default behavior skips BDFare fallback to avoid large per-query latency
    # on routes/dates that legitimately have no BS inventory.
    if st_ok and isinstance(st_rows, list) and not st_rows and not _env_true(ENV_FALLBACK_ON_EMPTY, default=False):
        return sharetrip_out

    LOG.warning("[BS] ShareTrip returned no usable rows; attempting BDFare fallback")
...
def _extract_data_config(html_text: str) -> Dict[str, Any]:
    ...
    if not match:
        raise ValueError("US-Bangla data-config not found in bootstrap HTML")
...
def _build_search_model(
    ...
    if origin_u not in airports or dest_u not in airports:
        raise ValueError(f"Unknown BS airport code(s): {origin_u}->{dest_u}")
...
def bootstrap_config(
    ...
    if resp.status_code != 200:
        raise RuntimeError(f"BS bootstrap failed status={resp.status_code}")
...
def _post_search(req: Requester, cfg: Dict[str, Any], model: Dict[str, Any]) -> tuple[int, Any]:
    ...
    if not rel_url:
        raise RuntimeError("BS SearchFlightsAction URL missing from bootstrap config")
...
def usbangla_search(
    ...
    try:
        ...
        LOG.error("[BS] bootstrap request failed: %s", exc)
    ...
    try:
        ...
        LOG.error("[BS] bootstrap parse failed: %s", exc)
    ...
    try:
        ...
        LOG.error("[BS] invalid search inputs: %s", exc)
    ...
    try:
        ...
        LOG.error("[BS] SearchFlights request failed: %s", exc)
    ...
    if _is_datadome_block(status_code, body):
        LOG.warning("[BS] DataDome challenge detected on SearchFlightsAction (status=%s)", status_code)
        output["raw"]["error"] = "datadome_blocked"
        output["raw"]["hint"] = (
            "US-Bangla TTInteractive search is protected by DataDome. "
            ...
            "BS_COOKIES_PATH and optionally BS_PROXY_URL."
        )
        return output
    ...
    if status_code == 200:
        ...
        if not output["rows"]:
            LOG.info("[BS] Search response received but no TTInteractive fare rows were found")
    return output
...
def fetch_flights(
    ...
    if source_mode == "amybd":
        ...
        if err == "search_not_ok" and "invalid login" in msg.lower():
            LOG.warning("[BS] AMYBD returned Invalid Login; falling back to ShareTrip")
