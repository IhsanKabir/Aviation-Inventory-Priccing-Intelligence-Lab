...
CREATE OR REPLACE VIEW `aeropulseintelligence.aviation_intel.vw_forecast_model_latest` AS
WITH latest_bundle AS (
  SELECT bundle_id
  FROM `aeropulseintelligence.aviation_intel.fact_forecast_bundle`
...
SELECT
  ...
  e.f1_macro
FROM `aeropulseintelligence.aviation_intel.fact_forecast_model_eval` e
JOIN `aeropulseintelligence.aviation_intel.fact_forecast_bundle` b
  ON b.bundle_id = e.bundle_id
JOIN latest_bundle lb
  ON lb.bundle_id = e.bundle_id;

CREATE OR REPLACE VIEW `aeropulseintelligence.aviation_intel.vw_forecast_route_latest` AS
WITH latest_bundle AS (
  SELECT bundle_id
  FROM `aeropulseintelligence.aviation_intel.fact_forecast_bundle`
  QUALIFY ROW_NUMBER() OVER (ORDER BY bundle_created_at_utc DESC, stamp DESC, bundle_name DESC) = 1
...
SELECT
  ...
  r.f1_macro
FROM `aeropulseintelligence.aviation_intel.fact_forecast_route_eval` r
JOIN `aeropulseintelligence.aviation_intel.fact_forecast_bundle` b
  ON b.bundle_id = r.bundle_id
JOIN latest_bundle lb
  ON lb.bundle_id = r.bundle_id;

CREATE OR REPLACE VIEW `aeropulseintelligence.aviation_intel.vw_forecast_route_winner_latest` AS
WITH latest_bundle AS (
  SELECT bundle_id
  FROM `aeropulseintelligence.aviation_intel.fact_forecast_bundle`
  QUALIFY ROW_NUMBER() OVER (ORDER BY bundle_created_at_utc DESC, stamp DESC, bundle_name DESC) = 1
...
SELECT
  ...
  r.candidate_models
FROM `aeropulseintelligence.aviation_intel.fact_forecast_route_winner` r
JOIN `aeropulseintelligence.aviation_intel.fact_forecast_bundle` b
  ON b.bundle_id = r.bundle_id
JOIN latest_bundle lb
  ON lb.bundle_id = r.bundle_id;

CREATE OR REPLACE VIEW `aeropulseintelligence.aviation_intel.vw_forecast_next_day_latest` AS
WITH latest_bundle AS (
  SELECT bundle_id
  FROM `aeropulseintelligence.aviation_intel.fact_forecast_bundle`
  QUALIFY ROW_NUMBER() OVER (ORDER BY bundle_created_at_utc DESC, stamp DESC, bundle_name DESC) = 1
...
FROM `aeropulseintelligence.aviation_intel.fact_forecast_next_day` n
JOIN `aeropulseintelligence.aviation_intel.fact_forecast_bundle` b
  ON b.bundle_id = n.bundle_id
JOIN latest_bundle lb
  ON lb.bundle_id = n.bundle_id;

CREATE OR REPLACE VIEW `aeropulseintelligence.aviation_intel.vw_backtest_eval_latest` AS
...
SELECT
  ...
  e.test_start,
  e.test_end
FROM `aeropulseintelligence.aviation_intel.fact_backtest_eval` e
JOIN `aeropulseintelligence.aviation_intel.fact_forecast_bundle` b
  ON b.bundle_id = e.bundle_id
JOIN latest_backtest lb
  ON lb.bundle_id = e.bundle_id;

CREATE OR REPLACE VIEW `aeropulseintelligence.aviation_intel.vw_backtest_route_winner_latest` AS
WITH latest_backtest AS (
  SELECT bundle_id
  FROM `aeropulseintelligence.aviation_intel.fact_forecast_bundle`
...
SELECT
  ...
  r.coverage_threshold_n,
  r.candidate_models
FROM `aeropulseintelligence.aviation_intel.fact_backtest_route_winner` r
JOIN `aeropulseintelligence.aviation_intel.fact_forecast_bundle` b
  ON b.bundle_id = r.bundle_id
JOIN latest_backtest lb
  ON lb.bundle_id = r.bundle_id;
